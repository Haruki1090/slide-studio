...
| `titleY` | 0.26 | タイトル |
| `headY` / `headH` | 0.56 / 0.80 | ヘッドメッセージ（最大 2 行） |
| `headRuleY` | 1.42 | 構造の区切り線（実線） |
| `bodyTop` | 1.62 | ボディ上端 |
| `bodyBottom` | 6.70 | ボディ下端 |
| `footRuleY` | 6.88 | フッタ区切り（点線） |
| `footY` | 6.94 | 出典・ページ番号 |

ボディの実効領域は **12.23 × 5.08 インチ**。ここに収め、**ここを使い切る。**
...
opts：`note`（脚注を空ける）、`split`（主領域の比率）、`gap`（既定 0.5）、`x` / `w` / `y` / `bottom`（入れ子にするとき）。
入れ子：親の矩形をそのまま渡す。`s.regions("two", r.bottom)` で上下型の下段を 2 列に、
`s.regions("top-bottom", { ...r.chart, gap: 0.3 })` で図の列を上下 2 段に切る（列の中で切るときも `x` / `w` が引き継がれる）。
